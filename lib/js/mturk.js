...
var crypto = Npm.require('crypto')
  ...
  , libxml = Npm.require('libxmljs')
  , util = Npm.require('util')
  , check = Npm.require('validator').check
  , sanitize = Npm.require('validator').sanitize
  ...
  mturk.CreateHIT = function(params, callback) {
    ...
    this.doRequest(params, function(err, doc){
      if(err) {
        ...
        if(HITId && HITTypeId) {
          callback(null, {'HITId': HITId.text(), 'HITTypeId': HITTypeId.text()});
        } else {
...
**************************************************************************/
  ...
  mturk.doRequest = function(params, callback) {
    ...
    request(url, function(error, response, xml) {
      ...
      if (error) {
        ...
        callback(params.Operation+': '+response.statusCode+' '+errMsg, null);
      } else {
        var doc = libxml.parseXml(xml);
  ...
  */
  function merge(obj1, obj2){
    var obj3 = {};
    // FIXME: remove Operation from obj2 if exists
